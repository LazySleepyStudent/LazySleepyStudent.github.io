...
let URL = "https://teachablemachine.withgoogle.com/models/Cw7nPirnM/";
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 2 && time <= 3 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >=3  && time <= 4 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if ((time >= 4 && time <= 6 && !poseState.firstWindowTriggered) ||
                ...
                break;
            case '4':
                if (time >= 6 && time <= 8 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 8 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/Cw7nPirnM/");
    }
}
