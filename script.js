...
let explosionSound = new Audio('Borf.mp3');
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 4 && time <= 7 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >=7  && time <= 9 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                ...
                break;
            case '4':
                if (time >= 12 && time <= 15 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 16 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
